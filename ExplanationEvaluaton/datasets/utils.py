...
def get_graph_data(path):
    ...
    if not os.path.exists(file_edge_labels):
        edge_labels = np.zeros(edges.shape[0]).astype(np.int32)
    if not os.path.exists(file_node_labels):
        node_labels = np.zeros(np.max(edges)).astype(np.int32)
    graph_id = 1
    starts = [1]
    node2graph = {}
...
def load_real_dataset(path_pkl, path_graph):
    ...
    #filter
    if False:
        filter=[ i for i,node_label in enumerate(node_label_lists) if len(node_label)<1000]
...
def reload_aids(path_pkl, path_graph):
    """Obtain the mutagenicity dataset from text files.
    ...
    :returns: adjecency matrix, node features, labels.
    """
    with open(path_graph+"/BBBP.pkl", 'rb') as fout:
        edge_lists, graph_labels, node_label_lists = pkl.load(fout)
