...
def load_natural_ground_truth(name, shuffle=True):
    """Load a the ground truth from the mutagenicity dataset.
    Mutag is a large dataset and can thus take a while to load into memory.
    ...
    :returns: np.array, np.array, np.array, np.array
    """
    print("Loading AIDS dataset, this can take a while")
    dir_path = os.path.dirname(os.path.realpath(__file__))
    path = dir_path + '/pkls/' + name + '.pkl'
    if not os.path.exists(path):  # pkl not yet created
        original_adjs, original_features, original_labels = load_real_dataset(path,
                                                                              dir_path + '/AIDS/AIDS_')
    else:
        with open(path, 'rb') as fin:
            original_adjs, original_features, original_labels = pkl.load(fin)

    print("Loading " + name + " groundtruth, this can take a while")
    path = dir_path + '/'+name+'/'+name+'_'
    edge_lists, _, edge_label_lists, _ = get_graph_data(path)
...
def load_dataset_ground_truth(_dataset, test_indices=None):
    ...
    if _dataset == "syn4":
        ...
        edge_index, labels, edge_list, edge_labels = load_natural_ground_truth("AIDS")
        selected = []
        np_edge_list = []
        for gid in range(0, len(edge_index)):
            ed = edge_list[gid]
        ...
        edge_index, labels, edge_list, edge_labels = load_natural_ground_truth(_dataset)
        selected = []
        np_edge_list = []
        for gid in range(0, len(edge_index)):
            ed = edge_list[gid]
